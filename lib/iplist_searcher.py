...
class IPlistSearcher(object):
    ...
    hostinfos = searcher.get_hostinfos('jp')
    print(hostinfos)
    """
    ...
    def _write_cache(self, cachedata):
    ...
    def _compare_cache(self, current_caches):
        ...
        current_caches = {
            ...
            'addresses': {
                'iplist1.xlsx': [[HostInfo(name, ipaddress), HostInfo(name, ipaddress)]],
                'iplist2.xlsx': [[HostInfo(name, ipaddress), HostInfo(name, ipaddress)]]
            }
        }
        ...
        for filename in current_filetimes.keys():
            if filename not in current_caches['filetimes']\
               or current_caches['filetimes'][filename] != current_filetimes[filename]:

                replaced = True
                new_cache_part['filetimes'][filename] = current_filetimes[filename]
            else:
                new_cache_part['filetimes'][filename] = current_caches['filetimes'][filename]
                if filename in current_caches['addresses']:
                    new_cache_part['addresses'][filename] = current_caches['addresses'][filename]
    ...
    def _create_cache(self, current_caches):
    ...
    def get_hostinfos(self):
        ...
        current_caches = self._read_cache()
        new_caches = self._create_cache(current_caches)
        self._write_cache(new_caches)
